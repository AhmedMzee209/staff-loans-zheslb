...
public class StaffProfileController {
    ...
    @Operation(
            ...
            description = "Creates a new staff profile with the provided details. Requires STAFF or ADMIN role.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Staff profile created successfully",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = StaffProfileDto.class))),
    ...
    public ResponseEntity<StaffProfileDto> createProfile(
            @Parameter(description = "Staff profile data to create", required = true)
            @Valid @RequestBody StaffProfileDto dto) {
        return ResponseEntity.ok(staffProfileService.createProfile(dto));
    }

    @Operation(
            summary = "Update staff profile",
            description = "Updates an existing staff profile with the provided details. Requires STAFF or ADMIN role.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<StaffProfileDto> updateProfile(
            @Parameter(description = "Staff profile ID", required = true, example = "123e4567-e89b-12d3-a456-426614174000")
            @PathVariable UUID id,
            @Parameter(description = "Updated staff profile data", required = true)
            @Valid @RequestBody StaffProfileDto dto) {
    ...
    @Operation(
            ...
            description = "Retrieves a specific staff profile by its unique identifier"
    )
    @ApiResponses(value = {
    ...
    @Operation(
            ...
            description = "Retrieves a list of all staff profiles in the system"
    )
    @ApiResponses(value = {
    ...
    @Operation(
            ...
            description = "Deletes a staff profile from the system. Requires ADMIN role.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses(value = {
