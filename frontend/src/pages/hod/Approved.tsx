...
const Approved: React.FC = () => {
  ...
  const approvedApplications = placeholderApplications.filter(app => app.status === 'ACCOUNTANT_REVIEW' || app.status === 'APPROVED');
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-medium text-gray-900">Approved Applications</h3>
      {approvedApplications.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          <FileText className="h-12 w-12 text-gray-400 mx-auto mb-4" />
          <p className="text-gray-600">No approved applications available</p>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {application.staff_member?.position || 'Position'}
                        </p>
                      </div>
                    </td>
